...
| Organization Identifier | `com.tkrite` |
| Bundle Identifier | `com.tkrite.SecureZip` |
| Interface | `SwiftUI` |
| Language | `Swift` |
...
| 項目 | 値 |
|------|-----|
| Identifier | `com.tkrite.SecureZip` |
| URL Schemes | `com.tkrite.SecureZip` （Google Cloud Console で設定した値） |
| Role | `Editor` |

> **注意**: Google Cloud Console での OAuth クライアント ID 設定時に
> macOS アプリ用のリダイレクト URI として `com.tkrite.SecureZip:/oauth2redirect` を追加してください。
